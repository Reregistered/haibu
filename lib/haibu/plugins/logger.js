...
logger.init = function (options, callback) {
  ...
  options.mongo  = options.mongo  || haibu.config.get('mongo');
...
logger.initInput = function (input, options) {
  ...
  // 
  // Add the mongoDB transport if options and a db name are specified.
  // 
  if (options.mongo && options.mongo.db) {
...
logger.logEvent = function (ev, level, msg, meta) {
  var name = ev.split(':');
      input = exports.namespaces[name[0]] || 'haibu',
      log = loggers[input];
  ...
  meta.collection = (name[0] === 'user') ? name[1] : input;
  log[level](ev, meta);
};
