...
function logApp(app, message) {
  return ['user', app.user, app.name, message].join(':')
}
//
  ...
  for (i = 0; i < required.length; i++) {
    ...
    for (i2 = 0; i2 < props.length; i2++) {
      if (typeof(check[props[i2]]) == 'undefined') {
        ...
        haibu.emit(logApp(app, 'repo:validate'), 'warn',  message, check);
...
Repository.prototype.installDependencies = function (callback) {
  ...
  if (this.app.dependencies) {
    return haibu.utils.npm.install(this.homeDir, this.app, callback);
  }
  ...
  fs.readFile(path.join(this.homeDir, 'package.json'), function (err, data) {
    if (!err) {
      try {
        ...
        self.app.dependencies = haibu.utils.mixin({}, pkg.dependencies, self.app.dependencies || {});
      }
      catch (err) {
        //
        // Ignore errors
    ...
    haibu.utils.npm.install(self.homeDir, self.app, callback);
  });
};
...
Repository.prototype.stat = function (callback) {
  ...
  fs.readdir(this.appDir, function (err, files) {
    if (err) {
      ...
      haibu.emit(logApp(self.app, 'repo:dir:notfound'), 'warn', { app: self.app.name, dir: self.appDir });
      callback(err, false);
    }
    else if (files.length === 0) {
      haibu.emit(logApp(self.app, 'repo:dir:empty'), 'warn', { app: self.app.name, dir: self.appDir });
      ...
      haibu.emit(logApp(self.app, 'repo:dir:exists'), 'info', { app: self.app.name, dir: sourceDir });
      callback(null, true);
    }
  });
};
...
Repository.prototype.mkdir = function (callback) {
  var self = this;
  haibu.emit(logApp(self.app, 'repo:dir:user:create'), 'info', { app: self.app.name, dir: this.userDir });

  // check if the user's folder already exists
  fs.stat(self.userDir, function (userErr, stats) {
    function createAppDir() {
      haibu.emit(logApp(self.app, 'repo:dir:app:create'), 'info', { app: self.app.name, dir: self.appDir });
      ...
      fs.stat(self.appDir, function (droneErr, stats) {
        if (droneErr) { // folder doesn't exist
          return fs.mkdir(self.appDir, 0755, function (mkAppErr) {
            if (mkAppErr) {
              haibu.emit(logApp(self.app, 'repo:dir:app:create'), 'error', { app: self.app.name, dir: self.appDir });
              return callback(mkAppErr, false);
            }

            // If we created the directory successfully callback with true
            haibu.emit(logApp(self.app, 'repo:dir:app:success'), 'info', { app: self.app.name, dir: self.appDir });
            callback(null, true);
          });
        }
    ...
    if (userErr) {
      return fs.mkdir(self.userDir, 0755, function (mkUserErr) {
        if (mkUserErr) {
          haibu.emit(logApp(self.app, 'repo:dir:user:create'), 'error', { app: self.app.name, dir: self.userDir });
          return callback(mkUserErr, false);
        }

        haibu.emit(logApp(self.app, 'repo:dir:user:success'), 'info', { app: self.app.name, dir: self.userDir });
        createAppDir();
      });
    }
