...
Spawner.prototype.spawn = function spawn (repo, callback) {
  ...
  haibu.emit(logApp(app, 'spawn:setup'), 'info', meta);
  ...
  Object.keys(haibu.activePlugins).forEach(function (plugin) {
    ...
    if (haibu.activePlugins[plugin].argv) {
      haibu.emit(logApp(app, 'plugin:argv'), 'info', {
        app: app.name,
        user: app.user,
        plugin: plugin
      });
  ...
  fs.stat(repo.startScript, function (err, stats) {
    ...
    haibu.emit(logApp(app, 'spawn:start'), 'info', {
    ...
    function onStdout (data) {
      haibu.emit(logApp(app, 'drone:stdout'), 'info', data.toString(), meta);
    }
    ...
    function onStderr (data) {
      data = data.toString()
      haibu.emit(logApp(app, 'drone:stderr'), 'error', data, meta);
